...
export default function AdminPostsBlog(props: IAdminPostsBlog) {
  ...
  // state
  const { filterPosts = [], active, snack, post } = useAdminPostsBlog(); // Инициализация filterPosts как пустого массива
  const dispatch = useAdminPostsBlogDispatch();

  // фильтрация
  useEffect(() => {
    if (Array.isArray(posts)) { // Проверка, что posts - это массив
      dispatch({
        type: "filter_posts",
        payload: posts.filter((el) =>
          Object.entries(el)
            .toString()
            .toLowerCase()
  ...
  const tableBody = Array.isArray(filterPosts) && filterPosts.length > 0 ? (
    filterPosts.map((el) => (
    ...
    <tr>
      <td colSpan={8} className={style.td}>Нет постов</td> {/* Заглушка для отсутствия постов */}
    </tr>
  );
      ...
      <div className={style.main}>
        <div className={style.wrappTable}>
          ...
          <table>
            ...
            <tbody>{tableBody}</tbody> {/* Отображение тела таблицы */}
          </table>
        </div>
      </div>
    </div>
